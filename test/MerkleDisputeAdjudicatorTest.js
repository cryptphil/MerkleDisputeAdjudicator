// This file is part of the MerkleDisputeAdjudicator.
...
function initMerkleTree(_numberLeaves) {
  ...
  // The MerkleDisputeAdjudicator finds the deviating leaf regardless of
  // whether only the deviating leaf is different (onlySingleDiffLeaf = true)
  // or every leaf after this one (onlySingleDiffLeaf = false).
  ...
  // Make sure that they are not equal (which is indeed very, very unlikely).
  while (nonce1 == nonce2) {
    nonce2 = web3.utils.randomHex(32);
  }
}
...
contract("MerkleDisputeAdjudicator", async accounts => {
  let collateral;
  let gasUsed = 0;
